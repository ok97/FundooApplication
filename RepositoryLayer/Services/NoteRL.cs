  ...
  public class NoteRL : INoteRL
    {
        ...
        public List<UserNoteResponseData> GetTrashedNotes(int userID)
        {
            try
            {
                List<UserNoteResponseData> userNoteLists = _userDBContext.Notes.
                    Where(user => user.UserId == userID && user.Trash == false).
        ...
        public List<UserNoteResponseData> GetArchievedNotes(int userID)
        {
            ...
            {
                List<UserNoteResponseData> userNoteLists = _userDBContext.Notes.
                    Where(user => user.UserId == userID && user.Archived == false).
                    Select(user => new UserNoteResponseData
                    {
                        NoteId = user.NotesId,
                        Title = user.Title,
        ...
        public List<UserNoteResponseData> GetPinnedNotes(int userID)
        {
            ...
            {
                List<UserNoteResponseData> userNoteLists = _userDBContext.Notes.
                    Where(user => user.UserId == userID && user.Pin == false).
                    Select(user => new UserNoteResponseData
                    {
                        NoteId = user.NotesId,
                        Title = user.Title,
        ...
        public bool TrashNote(int userId, int noteID)
        {
            ...
            {
                bool responseData;
                var userData = _userDBContext.Notes.FirstOrDefault(u => u.NotesId == noteID&&u.UserId==userId);
                if (userData.Trash == true)
                {
                    userData.Trash = false;
                   // userData.Pin = false;
